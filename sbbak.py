...
def incompatibilityFeatures(f):
    ...
    if f & 0x40:
        lst.append("Extents")
    if f & 0x80:
        lst.append("Fs Size 2^64 Blocks/64bit")
    if f & 0x100:
...
class SuperBlock():
    def __init__(self,data):
        ...
        self.clusterSize = 2^(10 + getU32(data,0x1C)) # if bigalloc feature is enabled otherwise is equal to blockSize
        ...
        self.groupDescriptorSize = getU16(data,0xfe)
    ...
    def blockGroups(self):
    	bg = self.blockGroups / self.blocksPerGroup
    	if(self.blocksPerGroup % self.blocksPerGroup != 0):
    		bg += 1
    	return bg
